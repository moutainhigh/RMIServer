...
import com.hgsoft.other.vo.receiptContent.prepaidC.PreCardStopRefundReceipt;
 ...
 */
@Service
public class PrepaidCUnifiedInterfaceService {

	private static final Logger logger = LoggerFactory.getLogger(PrepaidCUnifiedInterfaceService.class);
	...
	public String saveUnusable(PrepaidC prepaidC, PrepaidCBussiness prepaidCBussiness,
		...
		try {
			...
			//记录旧卡状态
			//String cardState = prepaidC.getState();
			...
			if (unifiedInterfaceService.savePrepaidCState(unifiedParam)) {
				...
				if("1".equals(serviceFlowRecord.getIsNeedBlacklist())){
					...
					//如果卡片为挂失卡，则先解除挂失黑名单
//					if(PrepaidCardStateEnum.loss.getIndex().equals(cardState)){
//						//保存黑名单流水解除挂失黑名单	给铭鸿
...
//					}
				...
				VehicleInfo vehicleInfo = vehicleInfoDao.findByPrepaidCardNo(prepaidCBussiness.getCardno());
				//清算接口
				if(result){//不需要下黑名单的
...
//							CardStateSendSerTypeEnum.cancelWithCard.getValue(), prepaidCBussiness.getCardno(), "22", userType);
					if(vehicleInfo!=null){
						cardObuService.saveCardUnusable(prepaidCBussiness.getTradetime(), Integer.parseInt(PrepaidCardStateEnum.cancel.getIndex()), CardStateSendSerTypeEnum.cancelWithCard.getValue(), prepaidCBussiness.getCardno(), "22", userType, 
								Integer.parseInt(vehicleInfo.getVehicleColor()), vehicleInfo.getVehiclePlate(), vehicleInfo.getNSCVehicleType());
					}
				}else{
					//无卡锁定或有卡锁定失败，写给铭鸿的清算数据：卡片状态信息
					if(vehicleInfo!=null){
						cardObuService.saveCardUnusable(prepaidCBussiness.getTradetime(), Integer.parseInt(PrepaidCardStateEnum.cancel.getIndex()), CardStateSendSerTypeEnum.cancelWithOutCard.getValue(), prepaidCBussiness.getCardno(), "22", userType, 
								Integer.parseInt(vehicleInfo.getVehicleColor()), vehicleInfo.getVehiclePlate(), vehicleInfo.getNSCVehicleType());
					}
//					cardObuService.saveCardStateInfo(prepaidCBussiness.getTradetime(), Integer.parseInt(PrepaidCardStateEnum.cancel.getIndex()),
//							CardStateSendSerTypeEnum.cancelWithOutCard.getValue(), prepaidCBussiness.getCardno(), "22", userType);
	...
	public String saveIssue(PrepaidC prepaidC, ElectronicPurse electronicPurse, CarObuCardInfo carObuCardInfo,
			PrepaidCBussiness prepaidCBussiness, MainAccountInfo mainAccountInfo, BillGet billGet, Map<String,Object> params) {
		/**
		...
		try {
			...
			if (unifiedInterfaceService.saveAccAvailableBalance(unifiedParam)) {
				...
				preCardIssuceReceipt.setHandleWay(ReceiptUtil.getHandleWay(params.get("authenticationType")+""));
				preCardIssuceReceipt.setPreCardNo(prepaidC.getCardNo());
				preCardIssuceReceipt.setPreCardFaceValue(NumberUtil.get2Decimal(prepaidC.getFaceValue().multiply(new BigDecimal("0.01"))));
				preCardIssuceReceipt.setPreCardCost(NumberUtil.get2Decimal(prepaidC.getCost_().multiply(new BigDecimal("0.01"))));
				preCardIssuceReceipt.setPreCardSaleFlag(this.saleTypeDetailDao.findSaleTypeNanmeByCode(prepaidC.getSaleFlag()));	//销售方式
				preCardIssuceReceipt.setPreCardSumCost(NumberUtil.get2Decimal(prepaidC.getFaceValue().add(prepaidC.getCost_()).multiply(new BigDecimal("0.01"))));
				preCardIssuceReceipt.setPreCardInvoicePrint(InvoicePrintEnum.getNameByValue(prepaidC.getInvoicePrint()));
				preCardIssuceReceipt.setPreCardSuit(ReceiptSuitEnum.getNameByValue(prepaidC.getSuit())==null?"无":ReceiptSuitEnum.getNameByValue(prepaidC.getSuit()));
				...
				receipt.setCardNo(preCardIssuceReceipt.getPreCardNo());
...
//-----------------------------------------------------------------------------------------------------
			...
			if (PrepaidCardBussinessTypeEnum.preCardImRegister.getValue().equals(prepaidCBussiness.getState())) {
				...
				if(!saveRechargeInfo(mainAccountInfo, newRechargeInfo, AccChangeTypeEnum.recharge, AccChangeTypeEnum.imRecharge)){
...
//----------------------------------------------------------------------------------------------------
				...
				if(PrepaidCardBussinessTypeEnum.preCardMemberRecharge.getValue().equals(prepaidCBussiness.getState())
					...
					preCardMemberRechargeReceipt.setPreCardBeforebalance(NumberUtil.get2Decimal(prepaidCBussiness.getBeforebalance().multiply(new BigDecimal("0.01"))));
					preCardMemberRechargeReceipt.setPreCardRealPrice(NumberUtil.get2Decimal(prepaidCBussiness.getRealprice().multiply(new BigDecimal("0.01"))));
					preCardMemberRechargeReceipt.setMainCountPreferentialBalance(NumberUtil.get2Decimal(prepaidCBussiness.getReturnMoney().multiply(new BigDecimal("0.01"))));
					preCardMemberRechargeReceipt.setPreCardTransferSum(NumberUtil.get2Decimal(prepaidCBussiness.getTransferSum().multiply(new BigDecimal("0.01"))));
					preCardMemberRechargeReceipt.setPreCardBalance(NumberUtil.get2Decimal(prepaidCBussiness.getBalance().multiply(new BigDecimal("0.01"))));
					receipt.setTypeCode(prepaidCBussiness.getState());
					receipt.setTypeChName(PrepaidCardBussinessTypeEnum.getNameByValue(prepaidCBussiness.getState()));
					...
					preCardImRegisterReceipt.setPreCardNo(prepaidCBussiness.getCardno());
					preCardImRegisterReceipt.setPreCardBeforebalance(NumberUtil.get2Decimal(prepaidCBussiness.getBeforebalance().multiply(new BigDecimal("0.01"))));
					preCardImRegisterReceipt.setPreCardRealPrice(NumberUtil.get2Decimal(prepaidCBussiness.getRealprice().multiply(new BigDecimal("0.01"))));
					preCardImRegisterReceipt.setMainCountPreferentialBalance(NumberUtil.get2Decimal(prepaidCBussiness.getReturnMoney().multiply(new BigDecimal("0.01"))));
					preCardImRegisterReceipt.setPreCardTransferSum(NumberUtil.get2Decimal(prepaidCBussiness.getTransferSum().multiply(new BigDecimal("0.01"))));
					preCardImRegisterReceipt.setPreCardBalance(NumberUtil.get2Decimal(prepaidCBussiness.getBalance().multiply(new BigDecimal("0.01"))));
					...
					receipt.setBusinessId(prepaidCBussiness.getId());
					receipt.setCardNo(preCardImRegisterReceipt.getPreCardNo());
		...
		try {
			...
			if (unifiedInterfaceService.saveAccAvailableBalance(unifiedParam)) {
				...
				if(PrepaidCardBussinessTypeEnum.preCardImRegisterCorrect.getValue().equals(oldPrepaidCBusiness.getState())){//免登陆充值冲正
					...
					saveRechargeInfo(mainAccountInfo, rechargeInfoForId, AccChangeTypeEnum.correct, AccChangeTypeEnum.imCorrect);
				...
				if(PrepaidCardBussinessTypeEnum.preCardRechargeCorrect.getValue().equals(oldPrepaidCBusiness.getState())){
					...
					preCardRechargeCorrectReceipt.setPreCardBeforebalance(NumberUtil.get2Decimal(oldPrepaidCBusiness.getBeforebalance().multiply(new BigDecimal("0.01"))));
					preCardRechargeCorrectReceipt.setCorrectMoney(NumberUtil.get2Decimal(oldPrepaidCBusiness.getRealprice().multiply(new BigDecimal("0.01"))).substring(1));
					preCardRechargeCorrectReceipt.setPreCardBalance(NumberUtil.get2Decimal(oldPrepaidCBusiness.getBalance().multiply(new BigDecimal("0.01"))));
					...
					preCardImRegisterCorrectReceipt.setPreCardBeforebalance(NumberUtil.get2Decimal(oldPrepaidCBusiness.getBeforebalance().multiply(new BigDecimal("0.01"))));
					preCardImRegisterCorrectReceipt.setCorrectMoney(NumberUtil.get2Decimal(oldPrepaidCBusiness.getRealprice().multiply(new BigDecimal("0.01"))).substring(1));
					preCardImRegisterCorrectReceipt.setPreCardBalance(NumberUtil.get2Decimal(oldPrepaidCBusiness.getBalance().multiply(new BigDecimal("0.01"))));
					...
					receipt.setCardNo(preCardImRegisterCorrectReceipt.getPreCardNo());
		...
		try {
			...
			if (unifiedInterfaceService.saveAccAvailableBalance(unifiedParam)) {
				...
				for(Map<String, String> addRegPair:addRegPairList){
					...
					addRegDetail.setOperId(customer.getOperId());
					addRegDetail.setOperNo(customer.getOperNo());
					addRegDetail.setOperName(customer.getOperName());
					addRegDetail.setPlaceId(customer.getPlaceId());
					addRegDetail.setPlaceNo(customer.getPlaceNo());
					addRegDetail.setPlaceName(customer.getPlaceName());
					addRegDetail.setOperTime(new Date());
...
//				prepaidCBussiness.setOperid(addReg.getSaleOperId());
				...
				addRegReceipt.setHandleWay(ReceiptUtil.getHandleWay(params.get("authenticationType")+""));
	 ...
	 * 领取新卡接口
	 * 
	 * @param mainAccountInfo 主账户信息，新储值卡，电子钱包，储值卡业务记录，客服流水
	 * @throws Exception
	 * @author zxy
	 */
	...
	public String saveGainCard(MainAccountInfo mainAccountInfo, PrepaidC prepaidC, PrepaidC newPrepaidC,ElectronicPurse electronicPurse, PrepaidCBussiness prepaidCBussiness, ServiceFlowRecord serviceFlowRecord,PrepaidCBussiness rechargePrepaidCBussiness, Map<String,Object> params) {
		...
		try {
			...
			if(newPrepaidC.getRealCost().compareTo(new BigDecimal("0"))==0){
				
				//2017/05/05 营运的回收接口增加了一个参数 newCardSourceType（产品类型）
				String newCardSourceType = "";
				Map<String, Object> newCardMap = inventoryService.getProductTypeByCode("1", newPrepaidC.getCardNo());//1是粤通卡
				...
				if (prepaidC.getCardNo().length()==16) {
					mapReclaim = inventoryService.omsInterface(prepaidC.getCardNo(), "2", interfaceRecordReclaim,"",
							prepaidCBussiness.getPlaceid(),prepaidCBussiness.getOperid(),prepaidCBussiness.getOperName(),"1","",null,new BigDecimal("0"),"211",newCardSourceType);
			...
			InterfaceRecord interfaceRecord = null;
			Map<String, Object> map = inventoryService.omsInterface(newPrepaidC.getCardNo(), "1", interfaceRecord,"issue",
					prepaidCBussiness.getPlaceid(),prepaidCBussiness.getOperid(),prepaidCBussiness.getOperName(),"1","customPoint",null,newPrepaidC.getRealCost(),"212","");
			boolean result = (Boolean) map.get("result");
			if (!result) {
				return map.get("message").toString();
			}
			...
			//2017/05/27
			/*领取新卡后，相关日期更新为：
			维保开始日期：若为有偿（工本费大于等于0）则为补领当天日期；若为无偿（工本费等于0），则沿用最近一张的发行日期
			有效启用日期、有效截止日期：一发时写入，无需更新。
			维保周期：2年，参数可配置；*/

			//设置有效起始时间与有效结束时间
			newPrepaidC.setStartDate((Date)map.get("startDate"));
			newPrepaidC.setEndDate((Date)map.get("endDate"));
			
			Calendar cal = Calendar.getInstance();;
			if(newPrepaidC.getRealCost().compareTo(new BigDecimal("0"))==0){
				//无偿，沿用原卡的发行时间作为维保起始时间
				cal.setTime(prepaidC.getSaleTime());
			}else{
			...
			newPrepaidC.setMaintainTime(cal.getTime());
			//设置id
			...
			unifiedParam.setPlaceName(prepaidCBussiness.getPlaceName());
			unifiedParam.setPlaceNo(prepaidCBussiness.getPlaceNo());
			// TODO the serviceFlow needs to move here
			if (unifiedInterfaceService.saveAccAvailableBalance(unifiedParam)) {
				unifiedParam.setType("5");
				unifiedParam.setPrepaidC(prepaidC);
				unifiedParam.setServiceFlowRecord(serviceFlowRecord);
				
				//由于挂起的卡片也能旧卡锁定，所以carObuCardInfo可能为空,vehicle也可能为空
				...
				if(carObuCardInfo != null){
					...
					// 车卡标签绑定
					//carObuCardInfoDao.updatePerPaidCID(newPrepaidC.getId(), prepaidC.getId());
				}
				...
				if(vehicle != null){
					...
					//这个时候要将新卡的状态改为挂起
					newPrepaidC.setState(PrepaidCardStateEnum.disabled.getIndex());
					//重新设置传参newPrepaidC
					unifiedParam.setNewPrepaidC(newPrepaidC);
				}
...
//					cardSecondIssuedDao.saveCardSecondIssued(cardSecondIssued);
					...
					if(vehicle != null){
						...
						//写给铭鸿的清算数据：用户状态信息
						// 旧卡注销
						cardObuService.saveUserStateInfo(prepaidCBussiness.getTradetime(), Integer.valueOf(UserStateInfoDealFlagEnum.unbindCarAndCard.getValue()), prepaidCBussiness.getOldCardno(), 
								"22", Integer.valueOf(vehicle.getVehicleColor()), vehicle.getVehiclePlate(), vehicle.getNSCVehicleType(), 
								null,obuSeq, obuIssueTime, obuExpireTime, "储值卡领取新卡后将旧卡注销");
						
						//写给铭鸿的清算数据：用户状态信息
						//新卡发行
						cardObuService.saveUserStateInfo(prepaidCBussiness.getTradetime(), Integer.valueOf(UserStateInfoDealFlagEnum.bindCarAndCard.getValue()), prepaidCBussiness.getCardno(), 
								"22", Integer.valueOf(vehicle.getVehicleColor()), vehicle.getVehiclePlate(), vehicle.getNSCVehicleType(), 
								null,obuSeq, obuIssueTime, obuExpireTime, "储值卡领取新卡");
						
						/*//旧卡注销
						//写给铭鸿的清算数据：卡片状态信息
						cardObuService.saveCardStateInfo(prepaidCBussiness.getTradetime(), Integer.parseInt(PrepaidCardStateEnum.cancel.getIndex()), 
								CardStateSendSerTypeEnum.cancelWithOutCard.getValue(), prepaidCBussiness.getOldCardno(), "22", userType);*/
						
						//新卡发行
						//写给铭鸿的清算数据：卡片状态信息
						cardObuService.saveCardStateInfo(prepaidCBussiness.getTradetime(), Integer.parseInt(CardStateSendStateFlag.nomal.getValue()), 
								CardStateSendSerTypeEnum.scIssue.getValue(), prepaidCBussiness.getCardno(), "22", userType);
					}else{
						...
						//新卡发行
						//写给铭鸿的清算数据：卡片状态信息
						cardObuService.saveCardStateInfo(prepaidCBussiness.getTradetime(), Integer.parseInt(PrepaidCardStateEnum.disabled.getIndex()), 
								CardStateSendSerTypeEnum.scIssue.getValue(), prepaidCBussiness.getCardno(), "22", userType);
					}
					...
					if(customer!=null)serviceWater.setUserNo(customer.getUserNo());
					if(customer!=null)serviceWater.setUserName(customer.getOrgan());
					serviceWater.setCardNo(prepaidCBussiness.getOldCardno());
					serviceWater.setNewCardNo(prepaidCBussiness.getCardno());
					serviceWater.setSerType("511");//储值卡领取新卡
					serviceWater.setAmt(newPrepaidC.getCost_());//应收金额
					serviceWater.setAulAmt(newPrepaidC.getRealCost());//实收金额
					serviceWater.setSaleWate(prepaidC.getSaleFlag());//销售方式
					serviceWater.setFlowState("1");//正常
					...
					serviceWater.setPlaceName(prepaidCBussiness.getPlaceName());
					serviceWater.setOperTime(new Date());
					serviceWater.setRemark("自营客服系统：储值卡领取新卡");
					serviceWaterDao.save(serviceWater);

					//储值卡换卡回执
					PreCardGetNewCardReceipt preCardGetNewCardReceipt = new PreCardGetNewCardReceipt();
					preCardGetNewCardReceipt.setTitle("储值卡换卡回执");
					preCardGetNewCardReceipt.setHandleWay(ReceiptUtil.getHandleWay(params.get("authenticationType")+""));
					preCardGetNewCardReceipt.setOldPreCardNo(prepaidCBussiness.getOldCardno());
					preCardGetNewCardReceipt.setPreCardNotCardStatus(prepaidCBussiness.getNotCardState());
					preCardGetNewCardReceipt.setPreCardNo(prepaidCBussiness.getCardno());
					preCardGetNewCardReceipt.setPreCardCost(NumberUtil.get2Decimal(newPrepaidC.getCost_().multiply(new BigDecimal("0.01"))));
					if("1".equals(preCardGetNewCardReceipt.getPreCardNotCardStatus())){
						preCardGetNewCardReceipt.setPreCardOldCardBalance(NumberUtil.get2Decimal(rechargePrepaidCBussiness.getRealprice().multiply(new BigDecimal("0.01"))));
					}else if("0".equals(preCardGetNewCardReceipt.getPreCardNotCardStatus())){
						preCardGetNewCardReceipt.setPreCardOldCardBalance("0.00");
						preCardGetNewCardReceipt.setExpireTime(DateUtil.formatDate(dbasCard.getExpireTime(),"yyyy-MM-dd HH:mm:ss"));
					}
					Receipt receipt = new Receipt();
					receipt.setTypeCode(PrepaidCardBussinessTypeEnum.preCardGetNewCard.getValue());
					receipt.setTypeChName(PrepaidCardBussinessTypeEnum.preCardGetNewCard.getName());
					receipt.setCardNo(preCardGetNewCardReceipt.getPreCardNo());
					this.saveReceipt(receipt,prepaidCBussiness,preCardGetNewCardReceipt,customer);
	...
	public String saveGainCardForAMMS(MainAccountInfo mainAccountInfo, PrepaidC prepaidC, PrepaidC newPrepaidC,ElectronicPurse electronicPurse, PrepaidCBussiness prepaidCBussiness, ServiceFlowRecord serviceFlowRecord,PrepaidCBussiness rechargePrepaidCBussiness, Map<String,Object> params) {
		...
		try {
			...
			/*补领后，卡片相关日期更新如下：
			有效启用时间、有效截止时间取营运系统一发时设置的时间；
			发行时间为：办理业务的当前时间
			维保开始时间：如果有偿：则使用本次发行时间；如果是无偿，则使用上一张卡的维保开始时间。
			维保周期：2年，参数可配置； */
...
//					cardSecondIssuedDao.saveCardSecondIssued(cardSecondIssued);
				...
				if(vehicle != null){
					...
					//新卡发行
					//写给铭鸿的清算数据：卡片状态信息
					cardObuService.saveCardStateInfo(prepaidCBussiness.getTradetime(), Integer.parseInt(PrepaidCardStateEnum.nomal.getIndex()), 
							CardStateSendSerTypeEnum.scIssue.getValue(), prepaidCBussiness.getCardno(), "22", userType);
				}else{
					//说明：如果vehicle为null，说明旧卡是挂起的卡去做旧卡锁定，领取的新卡也不会有绑定的车辆，所以领取的新卡为挂起状态
				...
				serviceWater.setPlaceName(prepaidCBussiness.getPlaceName());
				serviceWater.setOperTime(new Date());
				serviceWater.setRemark("代理点系统：储值卡领取新卡");
				serviceWaterDao.save(serviceWater);
	 ...
	 * 补领新卡接口
	 * 
	 * @param mainAccountInfo 主账户信息，旧储值卡，旧储值卡历史，新储值卡，电子钱包，旧电子钱包历史，储值卡业务记录，客服流水
	 * @throws Exception
	 * @author zxy
	 */
	...
	public String saveReplaceCard(MainAccountInfo mainAccountInfo, PrepaidC prepaidC, PrepaidC newPrepaidC,ElectronicPurse electronicPurse, PrepaidCBussiness prepaidCBussiness, ServiceFlowRecord serviceFlowRecord, Map<String,Object> params) {
		try {
			InterfaceRecord interfaceRecord = null;
			Map<String, Object> map = inventoryService.omsInterface(newPrepaidC.getCardNo(), "1", interfaceRecord,"issue",
					prepaidCBussiness.getPlaceid(),prepaidCBussiness.getOperid(),prepaidCBussiness.getOperName(),"1","customPoint",null,newPrepaidC.getRealCost(),"29","");
			boolean result = (Boolean) map.get("result");
			if (!result) {
				return map.get("message").toString();
			}
			...
			if (unifiedInterfaceService.saveAccAvailableBalance(unifiedParam)) {// 主账户信息
				...
				if(vehicle != null){
					...
					vehicleBussiness.setCardType(Constant.PREPAIDTYPE);//储值卡
					//vehicleBussiness.setTagNo(newTagInfo.getTagNo());
					vehicleBussiness.setType(VehicleBussinessEnum.prepaidCReplaceCard.getValue());//33: 储值卡补卡
					vehicleBussiness.setPlaceID(prepaidCBussiness.getPlaceid());
					vehicleBussiness.setOperID(prepaidCBussiness.getOperid());
					vehicleBussiness.setOperName(prepaidCBussiness.getOperName());
					vehicleBussiness.setOperNo(prepaidCBussiness.getOperNo());
				...
				if (unifiedInterfaceService.savePrepaidCState(unifiedParam)) {// 储值卡
					...
					BigDecimal PrePaidC_bussiness_NO = sequenceUtil.getSequence("SEQ_CSMS_PrePaidC_bussiness_NO");
					prepaidCBussiness.setId(Long.valueOf(PrePaidC_bussiness_NO.toString()));
					prepaidCBussiness.setBusinessId(newPrepaidC.getId());
					prepaidCBussinessDao.save(prepaidCBussiness);
					...
					if(vehicle != null){
						...
						cardObuService.saveUserStateInfo(prepaidCBussiness.getTradetime(), Integer.valueOf(UserStateInfoDealFlagEnum.unbindCarAndCard.getValue()), prepaidCBussiness.getOldCardno(), 
								"22", Integer.valueOf(vehicle.getVehicleColor()), vehicle.getVehiclePlate(), vehicle.getNSCVehicleType(), 
								null,obuSeq, obuIssueTime, obuExpireTime, "储值卡挂失补领后将旧卡注销");
						
						//写给铭鸿的清算数据：用户状态信息
						//新卡发行
						cardObuService.saveUserStateInfo(prepaidCBussiness.getTradetime(), Integer.valueOf(UserStateInfoDealFlagEnum.bindCarAndCard.getValue()), prepaidCBussiness.getCardno(), 
								"22", Integer.valueOf(vehicle.getVehicleColor()), vehicle.getVehiclePlate(), vehicle.getNSCVehicleType(), 
								null,obuSeq, obuIssueTime, obuExpireTime, "储值卡挂失后补领新卡");
						
						//旧卡注销
						//写给铭鸿的清算数据：卡片状态信息
						cardObuService.saveCardStateInfo(prepaidCBussiness.getTradetime(), Integer.valueOf(CardStateSendStateFlag.cancel.getValue()), 
								CardStateSendSerTypeEnum.cancelWithOutCard.getValue(), prepaidCBussiness.getOldCardno(), "22", userType);
...
//					cardSecondIssuedDao.saveCardSecondIssued(cardSecondIssued);
					...
					serviceWater.setCardNo(prepaidCBussiness.getOldCardno());
					serviceWater.setNewCardNo(prepaidCBussiness.getCardno());
					serviceWater.setSerType("509");//储值卡补领
					serviceWater.setAmt(newPrepaidC.getCost_());//应收金额
					serviceWater.setAulAmt(newPrepaidC.getRealCost());//实收金额
					serviceWater.setSaleWate(prepaidC.getSaleFlag());//销售方式
					serviceWater.setFlowState("1");//正常
					...
					serviceWater.setPlaceName(prepaidCBussiness.getPlaceName());
					serviceWater.setOperTime(new Date());
					serviceWater.setRemark("自营客服系统：储值卡补领");
					serviceWaterDao.save(serviceWater);

					//储值卡挂失补领回执
					PreCardReplaceMentReceipt preCardReplaceMentReceipt = new PreCardReplaceMentReceipt();
					preCardReplaceMentReceipt.setTitle("储值卡挂失补领回执");
					preCardReplaceMentReceipt.setHandleWay(ReceiptUtil.getHandleWay(params.get("authenticationType")+""));
					PrepaidCBussiness prepaidLossBusiness = this.prepaidCBussinessDao.findByCardNoAndState_(prepaidC.getCardNo(),PrepaidCardBussinessTypeEnum.preCardLoss.getValue());
					preCardReplaceMentReceipt.setLostReceiptNo(this.receiptDao.findByBusIdAndPTC(prepaidLossBusiness.getId(),ReceiptParentTypeCodeEnum.prepaidC.getValue()).getReceiptNo());	//挂失回执No
					preCardReplaceMentReceipt.setOldPreCardNo(prepaidCBussiness.getOldCardno());
					preCardReplaceMentReceipt.setPreCardNo(prepaidCBussiness.getCardno());
					preCardReplaceMentReceipt.setPreCardCost(NumberUtil.get2Decimal(newPrepaidC.getCost_().multiply(new BigDecimal("0.01"))));
					preCardReplaceMentReceipt.setExpireTime(DateUtil.formatDate(dbasCard.getExpireTime(),"yyyy-MM-dd HH:mm:ss"));
					Receipt receipt = new Receipt();
					receipt.setTypeCode(PrepaidCardBussinessTypeEnum.preCardReplaceMent.getValue());
					receipt.setTypeChName(PrepaidCardBussinessTypeEnum.preCardReplaceMent.getName());
					receipt.setCardNo(preCardReplaceMentReceipt.getPreCardNo());
					this.saveReceipt(receipt,prepaidCBussiness,preCardReplaceMentReceipt,customer);
	...
	public Map<String, Object> saveStopCard(ServiceFlowRecord serviceFlowRecord, PrepaidCBussiness prepaidCBussiness, Cancel cancel,Customer customer, PrepaidC prepaidC, Map<String,Object> params) {
		...
		try {
			...
			if (unifiedInterfaceService.savePrepaidCState(unifiedParam)) {
				...
				preCardCannelReceipt.setHandleWay(ReceiptUtil.getHandleWay(params.get("authenticationType")+""));
				...
				preCardCannelReceipt.setPreCardBalance(NumberUtil.get2Decimal(prepaidCBussiness.getBalance().multiply(new BigDecimal("0.01"))));
				receipt.setCardNo(preCardCannelReceipt.getPreCardNo());
	...
	public Map<String, Object> savePrepaidCRefund(PrepaidCBussiness prepaidCBussiness,Cancel cancel, RefundInfo refundInfo,Customer customer, PrepaidC prepaidC,Long bussinessPlaceId,Map<String,Object> params) {
		...
		try {
			...
			if(customer!=null)serviceWater.setUserName(customer.getOrgan());
			serviceWater.setCardNo(prepaidC.getCardNo());
			serviceWater.setSerType("531");//储值无卡终止使用
			//serviceWater.setAmt(prepaidC.getCost_());//应收金额
			//serviceWater.setAulAmt(prepaidC.getRealCost());//实收金额
			//serviceWater.setSaleWate(prepaidC.getSaleFlag());//销售方式
			//serviceWater.setFlowState("1");//正常
			...
			serviceWater.setPlaceName(prepaidCBussiness.getPlaceName());
			serviceWater.setOperTime(new Date());
			serviceWater.setRemark("自营客服系统：储值卡终止使用首次退款申请");
			serviceWaterDao.save(serviceWater);

			//储值卡退款申请回执
			PreCardStopRefundReceipt preCardStopRefundReceipt = new PreCardStopRefundReceipt();
			preCardStopRefundReceipt.setTitle("储值卡退款申请回执");
			preCardStopRefundReceipt.setHandleWay(ReceiptUtil.getHandleWay(params.get("authenticationType")+""));
			preCardStopRefundReceipt.setPreCardNo(prepaidC.getCardNo());
			preCardStopRefundReceipt.setBalance(NumberUtil.get2Decimal(prepaidCBussiness.getBalance().multiply(new BigDecimal("0.01"))));
			preCardStopRefundReceipt.setRefundBankNo(refundInfo.getBankNo());
			preCardStopRefundReceipt.setRefundBankMember(refundInfo.getBankMember());
			preCardStopRefundReceipt.setRefundBankOpenBranches(refundInfo.getBankOpenBranches());
			Receipt receipt = new Receipt();
			receipt.setTypeCode(prepaidCBussiness.getState());
			receipt.setTypeChName(PrepaidCardBussinessTypeEnum.getNameByValue(prepaidCBussiness.getState()));
			receipt.setCardNo(preCardStopRefundReceipt.getPreCardNo());
			this.saveReceipt(receipt,prepaidCBussiness,preCardStopRefundReceipt,customer);
	...
	public boolean updatePwd(PrepaidC prepaidC,PrepaidCBussiness prepaidCBussiness, Map<String,Object> params) {
		try {
			...
			preCardPasswordModify.setHandleWay(ReceiptUtil.getHandleWay(params.get("authenticationType")+""));
			...
			receipt.setCardNo(preCardPasswordModify.getPreCardNo());
	...
	public boolean resetPwd(PrepaidC prepaidC,PrepaidCBussiness prepaidCBussiness, Map<String,Object> params) {
		try {
			...
			preCardPasswordResetReceipt.setHandleWay(ReceiptUtil.getHandleWay(params.get("authenticationType")+""));
			...
			receipt.setCardNo(preCardPasswordResetReceipt.getPreCardNo());
	 ...
	 * @author zyh
	 */
	public boolean saveLock(PrepaidC prepaidC, Map<String,Object> params) {
		try {
			// 客服流水
			...
			if (unifiedInterfaceService.savePrepaidCState(unifiedParam)) {
				...
				preCardLossReceipt.setHandleWay(ReceiptUtil.getHandleWay(params.get("authenticationType")+""));
				...
				receipt.setBusinessId(prepaidCBussiness.getId());
				receipt.setCardNo(preCardLossReceipt.getPreCardNo());
	 ...
	 * @author zyh
	 */
	public boolean unLock(PrepaidC prepaidC, Map<String,Object> params) {
		try {
			// 客服流水
			ServiceFlowRecord serviceFlowRecord = new ServiceFlowRecord();
...
//				saveDarkList(prepaidC,darkList,"1", "0");
				...
				preCardUnLossReceipt.setHandleWay(ReceiptUtil.getHandleWay(params.get("authenticationType")+""));
				...
				receipt.setCardNo(preCardUnLossReceipt.getPreCardNo());
	...
	public void saveStart(PrepaidC prepaidC, CarObuCardInfo carObuCardInfo, VehicleBussiness vehicleBussiness, Map<String,Object> params) {
		try {
			...
			cardEnabledReceipt.setHandleWay(ReceiptUtil.getHandleWay(params.get("authenticationType")+""));
			cardEnabledReceipt.setCardNo(prepaidC.getCardNo());
			cardEnabledReceipt.setCardType("储值卡");
			cardEnabledReceipt.setCardSuit(ReceiptSuitEnum.getNameByValue(prepaidC.getSuit())==null?"无":ReceiptSuitEnum.getNameByValue(prepaidC.getSuit()));
			...
			serviceWater.setRemark("储值卡解除挂起");
			serviceWater.setPrepaidCBussinessId(prepaidCBussiness.getId());
			serviceWater.setVehicleBussinessId(vehicleBussiness.getId());
	...
	public void saveStop(PrepaidC prepaidC, CarObuCardInfo carObuCardInfo,VehicleBussiness vehicleBussiness,Map<String,Object> params) {
		try {
			...
			cardDisabledReceipt.setHandleWay(ReceiptUtil.getHandleWay(params.get("authenticationType")+""));
	...
	public void saveStart(AccountCInfo accountCInfo, CarObuCardInfo carObuCardInfo,VehicleBussiness vehicleBussiness,Map<String,Object> params) {
		try {
			...
			cardEnabledReceipt.setCardSuit(ReceiptSuitEnum.getNameByValue(accountCInfo.getSuit())==null?"无":ReceiptSuitEnum.getNameByValue(accountCInfo.getSuit()));
			cardEnabledReceipt.setVehiclePlate(vehicleInfo.getVehiclePlate());
			cardEnabledReceipt.setVehiclePlateColor(VehicleColorEnum.getName(vehicleInfo.getVehicleColor()));
			cardEnabledReceipt.setVehicleWeightLimits(vehicleInfo.getVehicleWeightLimits()+"");
			cardEnabledReceipt.setVehicleNSCvehicletype(NSCVehicleTypeEnum.getNameByValue(vehicleInfo.getNSCVehicleType()));
			...
			serviceWater.setSerType(ServiceWaterSerType.cardStart.getValue());
			serviceWater.setRemark("记帐卡解除挂起");
	...
	public void saveStop(AccountCInfo accountCInfo, CarObuCardInfo carObuCardInfo,VehicleBussiness vehicleBussiness,Map<String,Object> params) {
		try {
			...
			if(accountCInfo.getState().equals("1")){
				//原清算数据，没用了
				/*TollCardBlackDet tollCardBlackDet=new TollCardBlackDet(4401, null, accountCInfo.getCardNo(), null, " ", null,5, new Date(),0, new Date());
				TollCardBlackDetSend tollCardBlackDetSend=new TollCardBlackDetSend(4401, null, accountCInfo.getCardNo(), null, " ", null,5, new Date(),0, new Date());
				accountCService.saveTollCardBlack(accountCInfo, tollCardBlackDet, tollCardBlackDetSend);*/
			...
			if(accountCInfo.getState().equals("1")){
				...
				serviceWater.setRemark("记帐卡无卡挂起");
			}else{
				//有卡
				serviceWater.setSerType(ServiceWaterSerType.cardStop.getValue());//卡片有卡挂起
				serviceWater.setRemark("记帐卡有卡挂起");
			}
...
/*	@SuppressWarnings({ "unused", "null"})
	...
	/*private boolean saveRechargeInfo(RechargeInfo newRechargeInfo,MainAccountInfo mainAccountInfo){
		...
		newRechargeInfo.setBalance(tempAccount.getBalance());
		newRechargeInfo.setAvailableBalance(tempAccount.getAvailableBalance());
		...
		newRechargeInfo.setState(tempAccount.getState());
	...
	/*public boolean saveReversalRechargeInfo(MainAccountInfo mainAccountInfo,
						RechargeInfo newRechargeInfo, String flag){
		...
		UnifiedParam unifiedParam = new UnifiedParam();
		unifiedParam.setType(AccChangeTypeEnum.correct.getValue());
		unifiedParam.setMainAccountInfo(mainAccountInfo);
		unifiedParam.setNewRechargeInfo(newRechargeInfo);
		unifiedParam.setOperId(newRechargeInfo.getOperId());
		unifiedParam.setPlaceId(newRechargeInfo.getPlaceId());
		...
		unifiedParam.setPlaceName(newRechargeInfo.getPlaceName());
		unifiedParam.setPlaceNo(newRechargeInfo.getPlaceNo());
		unifiedParam.setFlag(AccChangeTypeEnum.imCorrect.getValue());

		if(!unifiedInterfaceService.saveAccAvailableBalance(unifiedParam)){
			return false;
		}

		if (rechargeInfoService.updateCorrectState(newRechargeInfo.getCorrectId()) != 1) {
		...
		newRechargeInfo.setAvailableRefundBalance(tempAccount.getAvailableRefundBalance());
		newRechargeInfo.setRefundApproveBalance(tempAccount.getRefundApproveBalance());
		newRechargeInfo.setMemo(AccChangeTypeEnum.imCorrect.getName());
		newRechargeInfo.setState(tempAccount.getState());
	...
	public boolean saveRechargeInfo(MainAccountInfo mainAccountInfo,
		RechargeInfo newRechargeInfo, AccChangeTypeEnum type, AccChangeTypeEnum flag){
		...
		UnifiedParam unifiedParam = new UnifiedParam();
		unifiedParam.setType(type.getValue());
		unifiedParam.setMainAccountInfo(mainAccountInfo);
		unifiedParam.setNewRechargeInfo(newRechargeInfo);
		unifiedParam.setOperId(newRechargeInfo.getOperId());
		unifiedParam.setPlaceId(newRechargeInfo.getPlaceId());
		...
		unifiedParam.setPlaceName(newRechargeInfo.getPlaceName());
		unifiedParam.setPlaceNo(newRechargeInfo.getPlaceNo());
		unifiedParam.setFlag(flag.getValue());

		if(!unifiedInterfaceService.saveAccAvailableBalance(unifiedParam)){
			throw new ApplicationException("更新账户金额失败");
		}

		MainAccountInfo tempAccount = mainAccountInfoDao.findById(mainAccountInfo.getId());

		if (AccChangeTypeEnum.imCorrect.getValue().equals(flag.getValue())) {
			if (rechargeInfoService.updateCorrectState(newRechargeInfo.getCorrectId()) != 1) {
				logger.error("主账户缴款记录[{}]发生变更，不能冲正", newRechargeInfo.getCorrectId());
				throw new ApplicationException("主账户缴款记录发生变更");
			}
		...
		newRechargeInfo.setAvailableRefundBalance(tempAccount.getAvailableRefundBalance());
		newRechargeInfo.setRefundApproveBalance(tempAccount.getRefundApproveBalance());
		newRechargeInfo.setMemo(flag.getName());
		newRechargeInfo.setState(tempAccount.getState());
		...
		try {
			...
			if (unifiedInterfaceService.saveAccAvailableBalance(unifiedParam)) {
				...
				for(Map<String, String> addMap:regPairAddList){
					...
					addRegDetailNew.setOperId(customer.getOperId());
					addRegDetailNew.setOperNo(customer.getOperNo());
					addRegDetailNew.setOperName(customer.getOperName());
					addRegDetailNew.setPlaceId(customer.getPlaceId());
					addRegDetailNew.setPlaceNo(customer.getPlaceNo());
					addRegDetailNew.setPlaceName(customer.getPlaceName());
					addRegDetailNew.setOperTime(now);
						...
						for(Map<String, String> updateMap:regPairUpdateList){
							...
							if(oldDetail.getId().toString().equals(updateMap.get("id"))){
								...
								if(!updateMap.get("cardNo").equals(oldDetail.getCardNo()) || BigDecimal.valueOf(Double.valueOf(updateMap.get("fee"))).compareTo(oldDetail.getFee().multiply(new BigDecimal("0.01"))) != 0){
									...
									oldDetail.setOperId(customer.getOperId());
									oldDetail.setOperNo(customer.getOperNo());
									oldDetail.setOperName(customer.getOperName());
									oldDetail.setPlaceId(customer.getPlaceId());
									oldDetail.setPlaceNo(customer.getPlaceNo());
									oldDetail.setPlaceName(customer.getPlaceName());
									oldDetail.setOperTime(now);
				...
				//更新明细后的总充值登记金额
				BigDecimal afterTotalFee = addRegDetailDao.findTotalFeeByAddRegId(addReg.getId());
				...
				if (afterTotalFee != null){
					addReg.setTotalFee(afterTotalFee);//新登记金额为原来的+改变的
				}else{
					addReg.setTotalFee(BigDecimal.ZERO);
				}
				...
				if(addRegDetails!=null){
					for(AddRegDetail temp : addRegDetails){
						...
						map.put("fee",NumberUtil.get2Decimal(temp.getFee().multiply(new BigDecimal("0.01"))));
						list.add(map);
					}
				}
				//储值卡充值登记修改回执
	...
	public String saveIssueForAMMS(PrepaidC prepaidC, ElectronicPurse electronicPurse, CarObuCardInfo carObuCardInfo,
		...
		try {
			...
			Map<String, Object> map = inventoryServiceForAgent.omsInterface(prepaidC.getCardNo(), "1", interfaceRecord,"issue",
					null,null,"","1","",null,prepaidC.getRealCost(),"21");
			boolean result = (Boolean) map.get("result");
			if (!result) {
				return map.get("message").toString();
			}
...
//			prepaidC.setEndDate((Date)map.get("endDate"));
			...
			prepaidC.setId(Long.valueOf(PrePaidC_NO.toString()));
			prepaidC.setBlackFlag(BlackFlagEnum.unblack.getValue());
			if("1".equals(prepaidC.getInvoicePrint())){//发票打印方式：充值即领
				prepaidC.setInvoiceChangeFlag(2l);//发票打印方式变更生效标志:生效
			}else{//发票打印方式：月结
				prepaidC.setInvoiceChangeFlag(0l);//发票打印方式变更生效标志:月结方式
			}
			prepaidCDao.save(prepaidC);
			...
			dbasCard.setApplyTime(new Date());
			dbasCard.setServiceId(prepaidCBussiness.getId());
			dbasCard.setCardAmt(new BigDecimal("0"));//卡片实际余额
			dbasCard.setReadFlag(DBACardFlowReadFlagEnum.abledRead.getValue());
			dbasCard.setEndFlag(DBACardFlowEndFlagEnum.arriComplete.getValue());
			dbasCard.setExpireFlag(DBACardFlowExpireFlagEnum.arriDispute.getValue());
			dbasCard.setEndServiceId(prepaidCBussiness.getId());
			dbasCard.setEndSerType(DBACardFlowEndSerTypeEnum.prepaidCCardIssuice.getValue());
			dbasCard.setEndCardAmt(new BigDecimal("0"));//完成金额
			dbasCard.setOperId(prepaidCBussiness.getOperid());
			dbasCard.setOperno(prepaidCBussiness.getOperNo());
			dbasCard.setOpername(prepaidCBussiness.getOperName());
			dbasCard.setPlaceId(prepaidCBussiness.getPlaceid());
			...
			vehicleBussiness.setCardType(Constant.PREPAIDTYPE);//储值卡
			//vehicleBussiness.setTagNo(newTagInfo.getTagNo());
			vehicleBussiness.setType(VehicleBussinessEnum.prepaidCIssue.getValue());//14：车辆储值卡发行
			vehicleBussiness.setPlaceID(prepaidCBussiness.getPlaceid());
			vehicleBussiness.setOperID(prepaidCBussiness.getOperid());
			vehicleBussiness.setOperName(prepaidCBussiness.getOperName());
			vehicleBussiness.setOperNo(prepaidCBussiness.getOperNo());
			...
			serviceWater.setPlaceNo(prepaidCBussiness.getPlaceNo());
			serviceWater.setPlaceName(prepaidCBussiness.getPlaceName());
			serviceWater.setRemark("代理点系统：储值卡发行");
			serviceWaterDao.save(serviceWater);
...
//				cardSecondIssuedDao.saveCardSecondIssued(cardSecondIssued);
			...
			Date obuExpireTime = null;
			//写给铭鸿的清算数据：用户状态信息
			cardObuService.saveUserStateInfo(prepaidCBussiness.getTradetime(), Integer.parseInt(UserStateInfoDealFlagEnum.bindCarAndCard.getValue()), prepaidC.getCardNo(), 
					"22", Integer.parseInt(vehicle.getVehicleColor()), vehicle.getVehiclePlate(), vehicle.getNSCVehicleType(), 
					null,obuSeq, obuIssueTime, obuExpireTime, "储值卡发行");
			
			//发行成功后，更新营运接口调用登记记录的客服状态,代理点不需要
			/*interfaceRecord = (InterfaceRecord) map.get("interfaceRecord");
			if (interfaceRecord != null&&interfaceRecord.getCardno()!=null) {
				interfaceRecord.setCsmsState("1");
				interfaceRecordDao.update(interfaceRecord);
...
//			return "储值卡发行失败，账户余额不足！";
	...
	public String delPrepaidCForAMMS(PrepaidC prepaidC, CarObuCardInfo carObuCardInfo, PrepaidCHis prepaidCHis,
			ElectronicPurse electronicPurse, ElectronicPurseHis electronicPurseHis, MainAccountInfo mainAccountInfo,
			PrepaidCBussiness prepaidCBussiness, BillGet billGet, BillGetHis billGetHis) {
		/**
		 * 删除处理： 1、删除对应卡车对应关系 2、增加电子钱包历史 3、删除电子钱包信息 4、增加储值卡历史记录 5、删除储值卡信息
		...
		try {
			...
			if (prepaidC.getCardNo().length()==16) {
				...
				map = inventoryServiceForAgent.omsInterface(prepaidC.getCardNo(), "1", interfaceRecord,"issue",
						null,null,"",
			...
			if (unifiedInterfaceService.saveAccAvailableBalance(unifiedParam)) {*/
				...
				serviceWater.setPlaceName(prepaidCBussiness.getPlaceName());
				serviceWater.setOperTime(new Date());
				serviceWater.setRemark("代理点系统：储值卡删除");
				serviceWaterDao.save(serviceWater);
		...
		try {
			...
			if (unifiedInterfaceService.savePrepaidCState(unifiedParam)) {// 储值卡
				...
				if(vehicle != null){
					//写给铭鸿的清算数据：用户状态信息
					// 旧卡注销
					cardObuService.saveUserStateInfo(prepaidCBussiness.getTradetime(), Integer.parseInt(UserStateInfoDealFlagEnum.unbindCarAndCard.getValue()), prepaidCBussiness.getOldCardno(), 
							"22", Integer.parseInt(vehicle.getVehicleColor()), vehicle.getVehiclePlate(), vehicle.getNSCVehicleType(), 
							null,obuSeq, obuIssueTime, obuExpireTime, "储值卡挂失补领后将旧卡注销");
					
					//写给铭鸿的清算数据：用户状态信息
					//新卡发行
					cardObuService.saveUserStateInfo(prepaidCBussiness.getTradetime(), Integer.parseInt(UserStateInfoDealFlagEnum.bindCarAndCard.getValue()), prepaidCBussiness.getCardno(), 
							"22", Integer.parseInt(vehicle.getVehicleColor()), vehicle.getVehiclePlate(), vehicle.getNSCVehicleType(), 
							null,obuSeq, obuIssueTime, obuExpireTime, "储值卡挂失后补领新卡");
...
//					cardSecondIssuedDao.saveCardSecondIssued(cardSecondIssued);
				...
				serviceWater.setPlaceName(prepaidCBussiness.getPlaceName());
				serviceWater.setOperTime(new Date());
				serviceWater.setRemark("代理点系统：储值卡补领");
				serviceWaterDao.save(serviceWater);
