...
@Component
public class TagMaintainDao extends BaseDao{
	@Resource
	...
	public Pager findTagMaintain(Pager pager, String tagNo, String vehicleColor, String vehiclePlate,
		...
		String sql = "select * from ( "
		            ...
		            +"t.tagstate, "
		            +"tr.MaintainType, "
		            +"tr.BackToCustomerTime BackToCustomerTime, "
		            +"tr.SendRepairTime SendRepairTime, "
		            +"tr.RepairBackTime RepairBackTime, "
		            +"tr.faultType faultType, "
		            +"tr.reason reason, "
		            +"tr.NoticeCustomerTime NoticeCustomerTime, "
		            +"IdentificationCode, "
		            +"row_number() over(partition by t.id order by tr.id desc) r "
		            +"from csms_tag_info t "
		            +"left join csms_carobucard_info c on t.id = c.tagid "
		            +"left join csms_vehicle_info v on c.vehicleid = v.id "
		            +"left join CSMS_TagMain_Record tr on tr.TagInfoID = t.id " 
		            +"where ((t.tagstate = '1' and v.id is not null) or (t.tagstate = '3' and MaintainType = 3 and BackToCustomerTime is null)) "
		            +"and t.clientid=?"
		            + "and not exists (select BACKUPTAGNO "
		            +"from csms_tagmain_record "
		            +"where BACKTOCUSTOMERTIME is null "
		            +"and BACKUPTAGNO = t.tagno) "
		            +") where r=1 ";
...
//		update(sql_2.toString());
	...
	public void updateTagInfo(Long tagInfoId,Long hisSeqId, int tagState) {
		...
		sql.append("TagState="+tagState+",");//电子标签状态为维修
		sql.append("hisseqid="+hisSeqId+",");
		sql.append("blackflag="+BlackFlagEnum.black.getValue()+",");
...
//		sql.append(FieldUtil.getFieldMap(TagMainRecord.class,temp).get("nameAndValue")+" where id="+tagMainRecord.getId());
	...
	public TagMainRecord findByTagMainRecord(TagMainRecord temp){
		...
		if (StringUtils.isBlank(condition)) {
			throw new ApplicationException("TagMaintainDao.findByTagMainRecord查询条件为空");
		}
	...
	public void updateMaintainBackToCustomerTime(TagMainRecord temp) {
		StringBuffer sql = new StringBuffer("update CSMS_TAGMAIN_RECORD set HisSeqID=?, BackOperID=?, BackToCustomerTime=sysdate ");
		sql.append(" where id =? ");
		this.jdbcUtil.getJdbcTemplate().update(sql.toString(),temp.getHisSeqID(), temp.getBackOperID(),temp.getId());
	}
