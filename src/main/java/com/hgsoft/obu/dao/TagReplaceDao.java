...
@Component
public class TagReplaceDao extends BaseDao{
	@Resource
	...
	public Pager findTagReplacesByPagerForAMMS(Pager pager,String tagNo, String vehicleColor,
			String vehiclePlate, String idType, String idCode, String endSixNo,Long customerID,String bankCode){
		String sql = " select allCard.* from (select nvl(p.cardNo,a.cardNo) allCardNo,ti.id as tagInfoId,ti.tagNo,vc.vehicleColor,vc.vehiclePlate,cus.organ,cus.idType,cus.idCode,ti.tagState,ti.issuetime,vc.identificationCode,ROWNUM as num" 
					+" from csms_tag_info ti join csms_carobucard_info coc on coc.tagid=ti.id join csms_vehicle_info vc on coc.vehicleId=vc.id"
					...
					+" where ti.tagState='1' and cus.id=?) allCard"
