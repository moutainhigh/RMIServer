...
import com.hgsoft.other.vo.receiptContent.tag.TagIssuceReceipt;
...
import com.hgsoft.ygz.common.ObuStatusEmeu;
...
public class TagInfoService implements ITagInfoService {
	...
	public Pager obuRecordTagInfoListForAMMS(Pager pager, Date starTime, Date endTime,
		...
		try {
			Pager maps = tagInfoDao.obuRecordTagInfoListForAMMS(pager, starTime,
	...
	public Map<String, Object> verifyTagInfo(String tagNo) {
		...
		if (tagTakeDetail != null) {//先提货后发行
			...
			if ("0".equals(tagTakeDetail.getTagState())) {// 未发行----先提货后发行
				...
				TagTakeInfo tagTakeInfo = tagTakeInfoDao.findById(tagTakeDetail.getMainID());
	...
	public Map<String,Object> save(TagInfo tagInfo, Customer customer,VehicleInfo vehicleInfo,Long productInfoId,Map<String,Object> params) {
		...
		try {
			...
			/**@author luningyun @description ygz data obu data save @date 2017-10-24 09:11 start*/
			
			realTransferService.ObuInfoTransfer(customer,tagInfo, vehicleInfo, OperationTypeEmeu.ADD.getCode(), ObuStatusEmeu.NORMAL.getCode());
			...
			serviceWater.setRemark("自营客服系统：电子标签发行");
			serviceWater.setOperTime(new Date());
			
			serviceWaterDao.save(serviceWater);
			
			//电子标签发行回执
			TagIssuceReceipt tagIssuceReceipt = new TagIssuceReceipt();
			tagIssuceReceipt.setTitle("电子标签发行回执");
			tagIssuceReceipt.setHandleWay(ReceiptUtil.getHandleWay(params.get("authenticationType")+""));
			tagIssuceReceipt.setTagNo(tagInfo.getTagNo());
			tagIssuceReceipt.setTagSaleType(saleTypeDetailDao.findSaleTypeNanmeByCode(tagInfo.getSalesType()));
			tagIssuceReceipt.setTagIssueType(TagIssueTypeEnum.getIdTypeEnum(tagInfo.getIssueType()).getName());
			tagIssuceReceipt.setTagChargeCost(NumberUtil.get2Decimal(tagInfo.getChargeCost().doubleValue()*0.01));
			tagIssuceReceipt.setVehiclePlate(vehicle.getVehiclePlate());
			tagIssuceReceipt.setVehiclePlateColor(VehicleColorEnum.getName(vehicle.getVehicleColor()));
			tagIssuceReceipt.setVehicleWeightLimits(vehicle.getVehicleWeightLimits()+"");
			tagIssuceReceipt.setVehicleEngineNo(vehicle.getVehicleEngineNo());
			tagIssuceReceipt.setVehicleModel(vehicle.getModel());
			tagIssuceReceipt.setVehicleType(VehicleTypeEnum.getIdTypeEnum(vehicle.getVehicleType()).getName());
			tagIssuceReceipt.setVehicleUserType(VehicleUsingTypeEnum.getName(vehicle.getVehicleUserType()));
			tagIssuceReceipt.setVehicleUsingNature(UsingNatureEnum.getName(vehicle.getUsingNature()));
			tagIssuceReceipt.setVehicleOwner(vehicle.getOwner());
			tagIssuceReceipt.setVehicleLong(vehicle.getVehicleLong()+"");
			tagIssuceReceipt.setVehicleWidth(vehicle.getVehicleWidth()+"");
			tagIssuceReceipt.setVehicleHeight(vehicle.getVehicleHeight()+"");
			tagIssuceReceipt.setVehicleNSCvehicletype(NSCVehicleTypeEnum.getNameByValue(vehicle.getNSCVehicleType()));
			tagIssuceReceipt.setVehicleIdentificationCode(vehicle.getIdentificationCode());
			tagIssuceReceipt.setVehicleAxles(vehicle.getVehicleAxles()+"");
			tagIssuceReceipt.setVehicleWheels(vehicle.getVehicleWheels()+"");
			tagIssuceReceipt.setInstallMan(tagInfo.getInstallmanName());
			...
			receipt.setTagNo(tagBusinessRecord.getTagNo());
			receipt.setVehicleColor(vehicle.getVehicleColor());
			receipt.setVehiclePlate(vehicle.getVehiclePlate());
			this.saveReceipt(receipt,tagBusinessRecord,tagIssuceReceipt,customer);
	...
	public Map<String,Object> saveForAMMS(TagInfo tagInfo, Customer customer,VehicleInfo vehicleInfo,Long productInfoId,Map<String,Object> params) {
		...
		try {
			...
			receipt.setTypeCode(TagBussinessTypeEnum.tagIssuce.getValue());
			receipt.setTypeChName(TagBussinessTypeEnum.tagIssuce.getName());
			receipt.setTagNo(tagIssuceReceipt.getTagNo());
			receipt.setVehicleColor(vehicle.getVehicleColor());
			receipt.setVehiclePlate(vehicle.getVehiclePlate());
			this.saveReceipt(receipt,tagBusinessRecord,tagIssuceReceipt,customer);
		...
		try {
			...
			if ("1".equals(tagInfo.getIssueType()) || "4".equals(tagInfo.getIssueType())) { // 提货发行一次完成、更换发行
				if (tagInfo.getTagNo().length()==16) {
					if(tagInfo.getChargeCost()!=null)price = tagInfo.getChargeCost().multiply(new BigDecimal("-1"));
					//TODO
					map = inventoryService.omsInterface(tagInfo.getTagNo(), "3", interfaceRecord,"",
							tagInfo.getIssueplaceID(),tagInfo.getOperID(),tagInfo.getOperName(),"2","",null,price,"48","");
					boolean result = (Boolean) map.get("result");
					if (!result) {
						return map.get("message").toString();
					}
				}
			}else if("2".equals(tagInfo.getIssueType())){
				//先提货后发型的，删除后，要更新提货明细表的状态为未发行
				tagTakeDetailDao.updateTagStateByTagNo("0", tagInfo.getTagNo());
			}else if("3".equals(tagInfo.getIssueType())){//备件发行
				//判断库存是否可冲正备件电子标签
				if(tagInfo.getTagNo()!=null){
					if(tagInfo.getChargeCost()!=null)price = tagInfo.getChargeCost().multiply(new BigDecimal("-1"));
					map = inventoryService.omsInterface(tagInfo.getTagNo(), "8", interfaceRecord,"",
							tagInfo.getIssueplaceID(),tagInfo.getOperID(),tagInfo.getOperName(),"2","",null,price,"48","");
					boolean result = (Boolean) map.get("result");
					if (!result) {
						return map.get("message").toString();
			...
			/**@author luningyun @description ygz del data save @date 2017-10-24 12:03 start*/
			
			  realTransferService.ObuInfoTransfer(customer,tagInfo, vehicle, OperationTypeEmeu.DEL.getCode(), ObuStatusEmeu.NORMAL.getCode());
			...
			serviceWater.setPlaceName(tagInfo.getPlaceName());
			serviceWater.setPlaceNo(tagInfo.getPlaceNo());
			serviceWater.setRemark("自营客服系统：电子标签删除");
			serviceWater.setOperTime(new Date());
...
//			return "电子标签删除失败";
	...
	public boolean updateTagWriteBackFlag(Long id,SysAdmin sysadmin,CusPointPoJo cusPointPoJo){
		try {
			...
			serviceWater.setRemark("自营客服系统：电子标签回写");
			serviceWaterDao.save(serviceWater);
...
//			MacaoCardCustomer macaoCardCustomer =macaoCardCustomerDao.getMacaoCardCustomerByTagNo(tagInfo.getTagNo());
	...
	public Map<String, Object> updateIsWriteObu(Long id) {
		...
		try {
			tagInfoDao.updateIsWriteObu(id);
		} catch (ApplicationException e) {
			map.put("result", "0");
	...
	public boolean checkTagBusinessRecord(String tagNo) {
		return tagBusinessRecordDao.checkDelete(tagNo);
	}
	...
	public TagInfo findByObuSerial(String obuSerial) {
		return tagInfoDao.findByObuSerial(obuSerial);
	}
}
