...
@Component
public class TagRecoverDaoACMS extends BaseDao{
	@Resource
	...
	public Pager findTagRecoversByPagerForAMMS(Pager pager,String tagNo, String vehicleColor,
		...
		StringBuffer sql = new StringBuffer("select * from (select vc.IdentificationCode IdentificationCode,cus.idtype idtype,cus.idcode idcode,ti.id as tagInfoId,ti.tagNo tagNo,vc.vehicleColor vehicleColor,vc.vehiclePlate vehiclePlate,cus.Organ Organ,ti.TagState TagState,ti.Issuetime Issuetime,ROWNUM as num ");
		sql.append(" from CSMS_CarObuCard_info coc join CSMS_Vehicle_Info vc on coc.vehicleID=vc.id join CSMS_Tag_info ti on coc.TagID=ti.id join CSMS_Customer cus on ti.clientid=cus.id ");
		sql.append(" where coc.prepaidcid is null and coc.accountcid is null and ti.tagState='1' and cus.id="+customerID);
		sql.append(" union all");
		sql.append(" select vc.IdentificationCode IdentificationCode,cus.idtype idtype,cus.idcode idcode,ti.id as tagInfoId,ti.tagNo tagNo,vc.vehicleColor vehicleColor,vc.vehiclePlate vehiclePlate,cus.Organ Organ,ti.TagState TagState,ti.Issuetime Issuetime,ROWNUM as num ");
		sql.append(" from CSMS_CarObuCard_info coc join CSMS_Vehicle_Info vc on coc.vehicleID=vc.id join CSMS_Tag_info ti on coc.TagID=ti.id join CSMS_Customer cus on ti.clientid=cus.id ");
		...
		sql.append(" where j.cardtype ='22' and j.bankno ="+ bankCode +" and ti.tagState='1' and cus.id="+customerID);
		sql.append(" union all");
		sql.append(" select vc.IdentificationCode IdentificationCode,cus.idtype idtype,cus.idcode idcode,ti.id as tagInfoId,ti.tagNo tagNo,vc.vehicleColor vehicleColor,vc.vehiclePlate vehiclePlate,cus.Organ Organ,ti.TagState TagState,ti.Issuetime Issuetime,ROWNUM as num ");
		sql.append(" from CSMS_CarObuCard_info coc join CSMS_Vehicle_Info vc on coc.vehicleID=vc.id join CSMS_Tag_info ti on coc.TagID=ti.id join CSMS_Customer cus on ti.clientid=cus.id ");
		...
		sql.append(" where j.cardtype ='23' and j.bankno ="+ bankCode +" and  ti.tagState='1' and cus.id="+customerID+") where 1=1 ");
		...
		pager = findByPages(sql.toString(), pager, Objects);
		return pager;
	}
