...
@Component
public class TagInfoMigrateDaoACMS extends BaseDao {
	...
	public Pager tagInfoList(Pager pager, TagInfo tagInfo, VehicleInfo vehicleInfo, Customer customer,
		...
		StringBuffer sql = new StringBuffer("select t.id,t.tagNo,t.issuetime,TagState,t.maintenancetime,v.id vid,v.vehicleColor,ca.VehicleID,v.vehiclePlate,ROWNUM as num "
...
//		}
	...
	public Pager tagInfoListForAMMS(Pager pager, TagInfo tagInfo, VehicleInfo vehicleInfo, Customer customer,
		...
		StringBuffer sql = new StringBuffer("select * from (select v.identificationCode identificationCode,t.ClientID ClientID,t.id,t.tagNo tagNo,t.issuetime issuetime,TagState,t.maintenancetime,v.id vid,v.vehicleColor vehicleColor,v.vehiclePlate vehiclePlate,ROWNUM as num ");
		sql.append(" from csms_tag_info t left join CSMS_CarObuCard_info ca on t.id = ca.TagID left join csms_vehicle_info v on ca.VehicleID = v.id  ");
		sql.append(" where ca.prepaidcid is null and ca.accountcid is null and (t.tagState=1 or t.tagState=2) and t.IssueType !=3  ");
		sql.append(" union all");
		sql.append(" select v.identificationCode identificationCode,t.ClientID ClientID,t.id,t.tagNo tagNo,t.issuetime issuetime,TagState,t.maintenancetime,v.id vid,v.vehicleColor vehicleColor,v.vehiclePlate vehiclePlate,ROWNUM as num ");
		sql.append(" from csms_tag_info t join CSMS_CarObuCard_info ca on t.id = ca.TagID left join csms_vehicle_info v on ca.VehicleID = v.id  ");
		sql.append(" join csms_prepaidc p on p.id=ca.prepaidcid join CSMS_joinCardNoSection j on substr(p.cardno, 0, length(p.cardno) - 1) between j.code and j.endcode ");
		sql.append(" where j.cardtype ='22' and j.bankno ="+ bankCode +" and (t.tagState=1 or t.tagState=2) and t.IssueType !=3  ");
		sql.append(" union all");
		sql.append(" select v.identificationCode identificationCode,t.ClientID ClientID,t.id,t.tagNo tagNo,t.issuetime issuetime,TagState,t.maintenancetime,v.id vid,v.vehicleColor vehicleColor,v.vehiclePlate vehiclePlate,ROWNUM as num ");
		sql.append(" from csms_tag_info t join CSMS_CarObuCard_info ca on t.id = ca.TagID left join csms_vehicle_info v on ca.VehicleID = v.id  ");
		sql.append(" join csms_accountc_info a on a.id=ca.accountcid join CSMS_joinCardNoSection j on substr(a.cardno, 0, length(a.cardno) - 1) between j.code and j.endcode");
		sql.append(" where j.cardtype ='23' and j.bankno ="+ bankCode +" and (t.tagState=1 or t.tagState=2) and t.IssueType !=3) where 1=1 ");
		SqlParamer params=new SqlParamer();
		if(tagInfo!=null){
			if(StringUtil.isNotBlank(tagInfo.getTagNo())){
		...
		//根据客户的id找到对应的标签,这个是必需的，要通过身份验证
		if(customer.getId()!=null){
			params.eq("ClientID", customer.getId());
		}
		if(vehicleInfo != null){
			//以下条件页面已经注释
			if(StringUtil.isNotBlank(vehicleInfo.getVehicleColor())){
		...
		if(StringUtil.isNotBlank(identificationCode6)){
			params.like("identificationCode", identificationCode6);
		}
		sql=sql.append(params.getParam());
		Object[] Objects= params.getList().toArray();
