...
@Component
public class TagStopDaoACMS extends BaseDao{
	@Resource
	...
	public Pager findTagStopsByPagerForAMMS(Pager pager,String tagNo, String vehicleColor,
		...
		StringBuffer sql = new StringBuffer("select * from (select vc.identificationCode identificationCode,ti.id tagInfoId,ti.tagNo tagNo,ti.tagState TagState,vc.vehicleColor vehicleColor,vc.vehiclePlate vehiclePlate,ti.Issuetime Issuetime,ti.clientid clientid,ti.writebackflag writebackflag,coc.vehicleID,ROWNUM as num");
		sql.append(" from CSMS_Tag_info ti left join CSMS_CarObuCard_info coc on coc.TagID=ti.id left join CSMS_Vehicle_Info  vc on coc.vehicleID=vc.id");
		sql.append(" where coc.accountcid is null and coc.prepaidcid is null and (ti.tagState='1'or ti.tagState='2') and ti.clientid= "+customerID );
		sql.append(" union all");
		sql.append(" select vc.identificationCode identificationCode,ti.id tagInfoId,ti.tagNo tagNo,ti.tagState TagState,vc.vehicleColor vehicleColor,vc.vehiclePlate vehiclePlate,ti.Issuetime Issuetime,ti.clientid clientid,ti.writebackflag writebackflag,coc.vehicleID,ROWNUM as num");
		sql.append(" from CSMS_Tag_info ti join CSMS_CarObuCard_info coc on coc.TagID=ti.id join CSMS_Vehicle_Info  vc on coc.vehicleID=vc.id");
		...
		sql.append(" where j.cardtype ='22' and j.bankno ="+ bankCode +" and (ti.tagState='1'or ti.tagState='2') and ti.clientid= "+customerID);
		sql.append(" union all");
		sql.append(" select vc.identificationCode identificationCode,ti.id tagInfoId,ti.tagNo tagNo,ti.tagState TagState,vc.vehicleColor vehicleColor,vc.vehiclePlate vehiclePlate,ti.Issuetime Issuetime,ti.clientid clientid,ti.writebackflag writebackflag,coc.vehicleID,ROWNUM as num");
		sql.append(" from CSMS_Tag_info ti join CSMS_CarObuCard_info coc on coc.TagID=ti.id join CSMS_Vehicle_Info  vc on coc.vehicleID=vc.id");
		...
		sql.append(" where j.cardtype ='23' and j.bankno ="+ bankCode +" and (ti.tagState='1'or ti.tagState='2') and ti.clientid= "+customerID+") where 1=1");
		/*String sql = "select tagInfoId,tagNo,TagState,vehicleColor,vehiclePlate," +
				" coc.vehicleID,ti.Issuetime,clientid,writebackflag,ROWNUM as num from ( "+
				" select ti.id as tagInfoId,tagNo,TagState,Issuetime,ti.clientid as clientid,writebackflag from CSMS_Tag_info ti " +
				" where (ti.tagState='1'or ti.tagState='2') and ti.clientid= "+customerID +
		...
		//List tagRecovers = queryList(sql,Objects);
		sql=sql.append(" order by Issuetime desc ");
